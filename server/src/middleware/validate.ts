...
export function validateQuery<T extends ZodSchema>(schema: T) {
  return (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Store validated values in a custom property that routes can access
      (req as Request & { validatedQuery: T['_output'] }).validatedQuery = result;
      next();
    } catch (error) {
      if (error instanceof ZodError) {
        const details = error.errors.map(err => ({
...
/**
 * Helper to get validated query params - use with validateQuery middleware
 */

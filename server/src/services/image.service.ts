...
class PexelsClient {
  ...
  async search(query: string, page: number = 1, perPage: number = 20): Promise<PexelsSearchResponse> {
    try {
      ...
      throw {
        code: 'API_ERROR',
        message: `Failed to search images: ${error.message || 'Unknown error'}`,
      } as ImageServiceError;
    }
  }
}

// Convert Pexels response to our standardized format
...
export const searchImages = async (options: ImageSearchOptions): Promise<ImageSearchResult[]> => {
  ...
  let pexels: PexelsClient;
  try {
    pexels = new PexelsClient();
  } catch (err) {
    throw err;
  }

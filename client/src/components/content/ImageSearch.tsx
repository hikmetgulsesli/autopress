...
import { Search, X, Image as ImageIcon, ExternalLink } from 'lucide-react';
  ...
  const searchImages = useCallback(async (searchQuery: string, pageNum: number = 1) => {
    ...
    try {
      ...
      if (axiosError.response?.status === 503) {
        setError('Image search service is not configured. Please add PEXELS_API_KEY to your environment.');
    ...
    <div className="space-y-4">
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <div className="absolute bottom-0 left-0 right-0 p-2">
                    <p className="text-xs text-white truncate">{image.photographer}</p>
                  </div>
                </div>
              </button>
            ))}
      ...
      {!loading && images.length === 0 && !query && (
        <div className="text-center py-8">
          ...
          <p className="text-text-muted">Search for images to add to your content</p>
        </div>
      )}
    </div>
  );

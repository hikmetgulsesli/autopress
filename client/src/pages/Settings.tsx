...
import type { UpdateProfileRequest, ChangePasswordRequest, NotificationSettings, ApiKeys } from '../types';

type TabId = 'profile' | 'general' | 'notifications' | 'security' | 'api-keys';
...
function ApiKeysTab() {
  ...
  const handleSave = async (key: keyof ApiKeys) => {
    try {
      ...
      setApiKeys((prev) => ({ ...prev, [key]: values[key] || '' }));
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-lg font-semibold text-text">API Anahtarları</h2>
          <p className="text-sm text-text-muted">Harici servisler için API anahtarlarını yapılandırın</p>
        </div>
      </div>
        ...
        {API_KEY_FIELDS.map((field) => (
          <div key={field.key} className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              {saved[field.key] && (
                <span className="text-xs text-success">Kaydedildi</span>
              )}
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="px-4 py-2.5 rounded-lg font-medium text-sm flex items-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer bg-primary-400 text-surface hover:bg-primary-500 disabled:bg-surface-alt disabled:text-text-muted"
              >
                {saving[field.key] ? (
      ...
      <div className="mt-8 p-4 rounded-lg border bg-surface-alt border-border">
        <h3 className="text-sm font-medium mb-2 text-text">Güvenlik Notu</h3>
